...
def main():
    parser = argparse.ArgumentParser(description='DSD^2 experiments')
    ...
    name_run = args.dataset+"_"+args.model
    ...
    if args.model == "VGG-like":
        model = VGGlikeModel(num_classes, args.delta, args.gamma).to(device)

    elif args.model == "ResNet-18" and (args.dataset != "Tiny-ImageNet-200" or args.dataset != "ImageNet"):
    ...
    ## CREATE CHECKPOINTS DIRECTORY IF DOES NOT EXIST YET
    if not os.path.exists("checkpoints/"):
        os.makedirs("checkpoints/")

    ## TRAIN DENSE MODEL: 
    for epoch in range(0,epochs):
        train_acc, train_loss  = train_one_epoch(model, train_loader, optimizer, loss_fn, device)
        test_acc, test_loss = eval(model, test_loader, loss_fn, device)
        scheduler.step()
    ...
    torch.save(model, "checkpoints/"+name_run)
    ...
    for i in range(1, 35):
        ...
        torch.save(model, "checkpoints/"+name_of_run)
